...
const PAKKET_OPTS = [
  { key: 'start', label: 'Basis', kleur: '#8B5CF6' },
  { key: 'volledig', label: 'Uitgebreid', kleur: '#10B981' },
] as const
  ...
  return (
    <View style={{ marginTop: 4 }}>
      ...
      <View style={s.blok}>
        ...
        <Text style={s.blokSub}>€ 79/maand basis · € 149/maand uitgebreid · 1 kraam per pakket · enkel actieve uitbaters</Text>
        <View style={{ marginTop: 12, gap: 9 }}>
...
function Uitbaters() {
  ...
  async function zetBetaald(id: string, betaald: boolean) {
    setLijst((l) => l.map((u) => u.id === id ? { ...u, betaald } : u))
    await supabase.rpc('mgmt_uitbater_betaald', { p_id: id, p_betaald: betaald })
  }
  async function zetPauze(id: string, gepauzeerd: boolean) {
    setLijst((l) => l.map((u) => u.id === id ? { ...u, gepauzeerd } : u))
    await supabase.rpc('mgmt_uitbater_pauze', { p_id: id, p_gepauzeerd: gepauzeerd })
    herlaad()
  }
          ...
          <View key={u.id} style={s.blok}>
            <View style={s.uTop}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.attrSub}>{u.email ?? '—'} · {u.kramen}/{u.max_attracties} kraam(en) · {u.pakket === 'volledig' ? 'Uitgebreid' : 'Basis'}</Text>
              </View>
            ...
            <View style={[s.statusRij, { marginTop: 8 }]}>
              <Pressable onPress={() => zetBetaald(u.id, !u.betaald)}
                style={[s.statusChip, u.betaald && { backgroundColor: C.green, borderColor: C.green }]}>
                <Text style={[s.statusChipT, u.betaald && { color: '#fff' }]}>{u.betaald ? 'Betaald \u2713' : 'Niet betaald'}</Text>
              </Pressable>
              <Pressable onPress={() => zetPauze(u.id, !u.gepauzeerd)}
                style={[s.statusChip, u.gepauzeerd && { backgroundColor: C.red, borderColor: C.red }]}>
                <Text style={[s.statusChipT, u.gepauzeerd && { color: '#fff' }]}>{u.gepauzeerd ? 'Gepauzeerd' : 'Pauzeer abo'}</Text>
              </Pressable>
            </View>
